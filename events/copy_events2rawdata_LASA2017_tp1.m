...
%List patients cohort 2017 session 1
aux= ls(source_path);
names=aux([3:end],:); 

for n=6%2:length(names)
    ...
    if sub<=9
        uulaa_events=strcat('sub-0',num2str(sub),'_ses-001_task-uulaa_acq-multiband_events.mat');
        tydyy_events=strcat('sub-0',num2str(sub),'_ses-001_task-tydyy_acq-multiband_events.mat');
    else
        uulaa_events=strcat('sub-',num2str(sub),'_ses-001_task-uulaa_acq-multiband_events.mat');
        tydyy_events=strcat('sub-',num2str(sub),'_ses-001_task-tydyy_acq-multiband_events.mat');
    end
    ...
    sub_path1=fullfile(source_path, (names(n,:)), [(names(n,:)) '_1'], 'func','Triggers');
    cd(sub_path1)
