function PostvsPre_Sing_memvsSing_along_Tydyy(subjects, outdir, varargin)
%PostvsPre_Sing_memvsSing_along_Tydyy
%----------------------------------------------------------------------------------------
...
p.addParameter('outfolder', 'Sing_mem>Sing_along', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_mem>sing_along_Tydyy_Post>Pre_' subjects(sbj).name];
