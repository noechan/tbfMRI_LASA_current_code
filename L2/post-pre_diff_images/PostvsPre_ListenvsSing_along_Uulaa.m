function PostvsPre_ListenvsSing_along_Uulaa(subjects, outdir, varargin)
%PostvsPre_ListenvsSing_along_Uulaa
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Listen>Sing_along_Uulaa_Post>Pre_' subjects(sbj).name];
