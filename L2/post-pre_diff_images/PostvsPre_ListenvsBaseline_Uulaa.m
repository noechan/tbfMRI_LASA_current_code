function PostvsPre_ListenvsBaseline_Uulaa(subjects, outdir, varargin)
%PostvsPre_ListenvsBaseline_Uulaa
%----------------------------------------------------------------------------------------
...
p.addParameter('outfolder', 'Listen>Baseline', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Listen>Baseline_Uulaa_Post>Pre_' subjects(sbj).name];
