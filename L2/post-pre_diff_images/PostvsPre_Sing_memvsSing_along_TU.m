function PostvsPre_Sing_memvsSing_along_TU(subjects, outdir, varargin)
%PostvsPre_Sing_memvsSing_along_TU
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if subjects(sbj).group==1
        ...
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_mem>sing_along'}));
        ...
        cd(sub_path_post)
        load('SPM.mat')
        con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_mem>sing_along'}));
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_mem>sing_along_Post>Pre_' subjects(sbj).name];
