function PostvsPre_ListenvsBaseline_TU(subjects, outdir, varargin)
%PostvsPre_ListenvsBaseline_TU
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Listen>Baseline_Post>Pre_' subjects(sbj).name];
