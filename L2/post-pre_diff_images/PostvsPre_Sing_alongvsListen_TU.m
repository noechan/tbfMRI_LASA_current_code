function PostvsPre_Sing_alongvsListen_TU(subjects, outdir, varargin)
%PostvsPre_Sing_alongvsListen_TU
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_along>listen_Post>Pre_' subjects(sbj).name];
