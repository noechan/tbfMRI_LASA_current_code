function PostvsPre_Sing_memvsListen_Uulaa(subjects, outdir, varargin)
%PostvsPre_Sing_memvsListen_Uulaa
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_mem>listen_Uulaa_Post>Pre_' subjects(sbj).name];
