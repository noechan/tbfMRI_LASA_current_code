function PostvsPre_ListenvsBaseline_Tydyy(subjects, outdir, varargin)
%PostvsPre_ListenvsBaseline_Tydyy
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Listen>Baseline_Tydyy_Post>Pre_' subjects(sbj).name];
