function PostvsPre_ListenvsSing_mem_TU(subjects, outdir, varargin)
%PostvsPre_ListenvsSing_mem_TU
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Listen>Sing_mem_Post>Pre_' subjects(sbj).name];
