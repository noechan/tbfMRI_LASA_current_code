function PostvsPre_Sing_memvsListen_Tydyy(subjects, outdir, varargin)
%PostvsPre_Sing_memvsListen_Tydyy
%----------------------------------------------------------------------------------------
...
p.addParameter('outfolder', 'Sing_mem>Listen', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_mem>listen_Tydyy_Post>Pre_' subjects(sbj).name];
