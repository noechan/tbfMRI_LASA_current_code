function PostvsPre_Sing_alongvsSing_mem_Tydyy(subjects, outdir, varargin)
%PostvsPre_Sing_alongvsSing_mem_Tydyy
%----------------------------------------------------------------------------------------
...
p.addParameter('outfolder', 'Sing_along>Sing_mem', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_along>sing_mem_Tydyy_Post>Pre_' subjects(sbj).name];
