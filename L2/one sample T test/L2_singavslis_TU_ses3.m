...
L2_folder='singa_vs_listen_TU_ses3_wo14_27';
%% Prepare inputs
n=1;
for sbj = 1:size(subjects_s3, 1) %Dataset from session 3
    ...
    con_idx=find(ismember({SPM.xCon.name}, {'Sing_along>listen'}));
    if ~isempty(con_idx)
        if con_idx>9
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Sing along vs Listen TU';
