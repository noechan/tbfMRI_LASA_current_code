...
L2_folder='listening_vs_singm_TU_ses2';
%% Prepare inputs
n=1;
for sbj = 1:size(subjects_s2, 1) %Dataset from session 2
    disp(subjects(sbj).name)
    sub_path= fullfile(subjects_s2(sbj).folder, subjects_s2(sbj).name, ses, der,L1, L1_folder);
    cd(sub_path), load('SPM.mat')
    con_idx=find(ismember({SPM.xCon.name}, {'Listen>sing_mem'}));
    if ~isempty(con_idx)
        if con_idx>9
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Listening vs Sing from memory TU';
