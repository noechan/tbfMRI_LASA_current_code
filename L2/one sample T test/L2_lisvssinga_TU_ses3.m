...
L2_folder='listening_vs_singa_TU_ses3';
%% Prepare inputs
n=1;
for sbj = 1:size(subjects_s3, 1) %Dataset from session 3
    ...
    con_idx=find(ismember({SPM.xCon.name}, {'Listen>sing_along'}));
    if ~isempty(con_idx)
        if con_idx>9
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Listening vs Sing along TU';
