function L2_PairedT_SingmVSRest_TU_Age(subjects, outdir, varargin)
...
p.addParameter('L2folder', 'singm_vs_rest_TU_Age_TIV_wo_motion_outliers', @ischar)
...
sub_path_pre= fullfile(subjects(sbj).folder, subjects(sbj).name, ses_pre, Arg.derdir,Arg.L1dir, Arg.L1folder);
    cd(sub_path_pre), load('SPM.mat')
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_mem>baseline'}));
        ...
        con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_mem>baseline'}));
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Sing mem vs Rest TU AB/BA Pre vs Post';
matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = 'Sing mem vs Rest TU AB/BA Post vs Pre';
