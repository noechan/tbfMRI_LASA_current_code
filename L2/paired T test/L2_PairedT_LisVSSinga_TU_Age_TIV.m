function L2_PairedT_LisVSSinga_TU_Age(subjects, outdir, varargin)
...
p.addParameter('L2folder', 'listen_vs_singa_TU_Age_TIV_wo_motion_outliers_wo_14', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    cd(sub_path_pre), load('SPM.mat')
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Listen>sing_along'}));
        ...
        con_idx_post=find(ismember({SPM.xCon.name}, {'Listen>sing_along'}));
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Listening vs Sing along TU AB/BA Pre vs Post';
matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = 'Listening vs Sing along TU AB/BA Post vs Pre';
