...
p.addParameter('L2folder', 'singm_vs_listen_TU_Age_TIV_wo_motion_outliers', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    cd(sub_path_pre), load('SPM.mat')
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_mem>listen'}));
        ...
        con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_mem>listen'}));
...
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = 'Sing mem vs Listen TU AB/BA Post vs Pre';
