function L2_PairedT_SingmVSListen_Trained_age(subjects, outdir, varargin)
...
p.addParameter('contrast_folder', 'Sing_mem>Listen', @ischar)
p.addParameter('L2folder', 'singm_vs_listen_TrainedvsUntrained_Age_wo_motion_outliers', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if subjects(sbj).group==1 % Trained Song Uulaa;
        postvspre_trained{sbj,1}=spm_select('List', fullfile(Arg.datadir,Arg.Uulaa_folder,Arg.contrast_folder),['^Sing_mem>listen_Uulaa_Post>Pre_' subjects(sbj).name '.*\.nii$']);
        postvspre_trained_fnames{sbj,1}=fullfile(Arg.datadir,Arg.Uulaa_folder,Arg.contrast_folder,postvspre_trained{sbj,1});
        postvspre_untrained{sbj,1}=spm_select('List', fullfile(Arg.datadir,Arg.Tydyy_folder,Arg.contrast_folder),['^Sing_mem>listen_Tydyy_Post>Pre_' subjects(sbj).name '.*\.nii$']);
    ...
    elseif subjects(sbj).group==2 % Trained Song Tydyy
        postvspre_trained{sbj,1}=spm_select('List', fullfile(Arg.datadir,Arg.Tydyy_folder,Arg.contrast_folder),['^Sing_mem>listen_Tydyy_Post>Pre_' subjects(sbj).name '.*\.nii$']);
        postvspre_trained_fnames{sbj,1}=fullfile(Arg.datadir,Arg.Tydyy_folder,Arg.contrast_folder,postvspre_trained{sbj,1});
        postvspre_untrained{sbj,1}=spm_select('List', fullfile(Arg.datadir,Arg.Uulaa_folder,Arg.contrast_folder),['^Sing_mem>listen_Uulaa_Post>Pre_' subjects(sbj).name '.*\.nii$']);
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Sing mem vs Listen Trained vs Untrained AB/BA';
matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = 'Sing mem vs Listen Untrained vs Trained AB/BA';
