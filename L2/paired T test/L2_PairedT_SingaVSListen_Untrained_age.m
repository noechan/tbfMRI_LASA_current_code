function L2_PairedT_SingaVSListen_Untrained_age(subjects, outdir, varargin)
...
p.addParameter('L2folder', 'singa_vs_listen_Untrained_Age_wo_motion_outliers_wo_14', @ischar)
...
for sbj = 1:size(subjects, 1)
    ...
    if subjects(sbj).group==1 % Untrained Song Tydyy
        ...
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_along>listen Tydyy'}));
        ...
        cd(sub_path_post)
        load('SPM.mat')
        con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_along>listen Tydyy'}));
    ...
    elseif subjects(sbj).group==2 % Untrained Song Uulaa
        ...
        sub_path_pre= fullfile(subjects(sbj).folder, subjects(sbj).name, ses_pre, Arg.derdir,Arg.L1dir, Arg.L1folder);
        cd(sub_path_pre), load('SPM.mat')
        con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_along>listen Uulaa'}));
        if ~isempty(con_idx_pre)
            if con_idx_pre >9
                con_pre{sbj,1}=spm_select('List', fullfile(sub_path_pre),['^con_00' num2str(con_idx_pre) '.*\.nii$']);
        ...
        con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_along>listen Uulaa'}));
        if ~isempty(con_idx_post)
            if con_idx_post >9
                con_post{sbj,1}=spm_select('List', fullfile(sub_path_post),['^con_00' num2str(con_idx_post) '.*\.nii$']);
            elseif con_idx_post <10
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Sing along vs Listen Untrained AB/BA Pre vs Post';
matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = 'Sing along vs Listen Untrained AB/BA Post vs Pre';
